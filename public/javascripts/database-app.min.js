 ...
 * Module dependencies.
 */
    var Emitter = require("emitter"), classes = require("classes");
    /**
 * Expose `Overlay`.
 */
...
require.register("eugenicsarchivesca-has-css-animations/index.js", function(exports, require, module) {
    ...
    var animation = false, animationstring = "animation", keyframeprefix = "", domPrefixes = "Webkit Moz O ms Khtml".split(" "), pfx = "";
...
require.register("bmcmahen-animate-css/index.js", function(exports, require, module) {
    var hasAnimations = require("has-css-animations"), classes = require("classes"), cssEvent = require("css-emitter"), once = require("once");
    // API:
 ...
 *
 */
    var Emitter = require("emitter"), Overlay = require("overlay"), classes = require("classes"), animate = require("animate-css");
    ...
    Modal.prototype.show = function() {
        ...
        this.emit("show");
        this.overlay.show();
        var cls = classes(this.el), _this = this;
        cls.add("in");
...
require.register("bmcmahen-ordered-dictionary/index.js", function(exports, require, module) {
    // Modules
    var indexOf = require("indexof"), Emitter = require("emitter");
    var OrderedDictonary = function(attr) {
        if (!(this instanceof OrderedDictonary)) return new OrderedDictonary(attr);
        this.map = {};
        this.array = [];
        if (typeof attr === "object") this.set(attr);
    };
    ...
    OrderedDictonary.prototype.set = function(key, val) {
        var attr, attrs;
        if (typeof key === "object") attrs = key; else (attrs = {})[key] = val;
        for (attr in attrs) {
            if (attr in this.map) this.map[attr] = attrs[attr]; else {
...
require.register("bmcmahen-cast/index.js", function(exports, require, module) {
    // Required Modules.
    var Emitter = require("emitter"), clone = require("clone"), bind = require("bind"), type = require("type"), OrderedDictionary = require("ordered-dictionary"), indexOf = require("indexof"), translate = require("translate");
    ...
    Cast.prototype.data = function(attr, fn) {
        ...
        var len = this.collection.length(), keys = [];
        ...
        for (var i = 0, l = attr.length; i < l; i++) {
            ...
            keys.push(key);
            if (model) model.set(attr[i]); else this.collection.set(key, new Block(attr[i], this));
        }
    ...
    Cast.prototype.justify = function(options) {
        ...
        var cw = this.wrapperWidth, w = this.boxWidth, pw = this.paddingWidth, ph = this.paddingHeight, h = this.boxHeight;
        var bpr = Math.floor((cw - pw * 2) / (w + pw));
        var getLeft = function(c, r) {
            if (c === 0) return 0;
            if (c === bpr - 1) return cw - w;
            var remainingSpace = cw - w * bpr, padding = remainingSpace / (bpr - 1);
            return w + c * padding + (c - 1) * w;
        };
        this.collection.forEach(function(key, model, i) {
            var r = Math.floor(i / bpr), c = i % bpr, left = getLeft(c, r), top = r * h + (r + 1) * ph;
            model.set({
                left: left,
                top: top,
                width: w,
    ...
    Cast.prototype.center = function(options) {
        if (options) this.setOptions(options);
        var cw = this.wrapperWidth, w = this.boxWidth, h = this.boxHeight, pw = this.paddingWidth, ph = this.paddingHeight;
        var bpr = Math.floor(cw / (w + pw));
        var mx = (cw - bpr * w - (bpr - 1) * pw) * .5;
        this.collection.forEach(function(key, model, i) {
            var r = Math.floor(i / bpr), c = i % bpr, left = mx + c * (w + pw), top = r * h + (r + 1) * ph;
            model.set({
                left: left,
                top: top,
                width: w,
    ...
    Cast.prototype.list = function(options) {
        if (options) this.setOptions(options);
        var h = this.boxHeight, ph = this.paddingHeight;
        this.collection.forEach(function(id, model, i) {
            var top = (h + ph) * i;
            model.set({
                left: 0,
    ...
    Cast.prototype.sortBy = function(field, invert) {
        ...
        this.collection.sort(function(left, right) {
            var leftVal = left.get(field), rightVal = right.get(field);
    ...
    var CastItemView = function(options) {
        ...
        this.model.on("change:attribute", bind(this, this.render)).on("destroy", bind(this, this.remove));
    ...
    CastItemView.prototype.remove = function() {
        this.model.off("change:attribute").off("destroy");
    ...
    CastItemView.prototype.changePosition = function() {
        var top = this.model.get("top"), left = this.model.get("left"), style = this.el.style, width = this.model.get("width"), height = this.model.get("height");
    ...
    CastItemView.prototype.showOrHide = function() {
        var _this = this, el = this.el, style = el.style;
 ...
 */
    Swipe.prototype.transitionDuration = function(ms) {
        ...
        s.webkitTransitionDuration = s.MozTransitionDuration = s.msTransitionDuration = s.OTransitionDuration = s.transitionDuration = ms + "ms";
    };
    /**
 * Translate to `x`.
 *
 ...
 * Module dependencies.
 */
    var proto = require("./proto"), statics = require("./static"), Emitter = require("emitter");
    /**
 * Expose `createModel`.
 */
 ...
 * Module dependencies.
 */
    var request = require("superagent"), Collection = require("collection"), noop = function() {};
    /**
 ...
 * Module dependencies.
 */
    var Emitter = require("emitter"), request = require("superagent"), JSON = require("json"), each = require("each"), noop = function() {};
    /**
 * Mixin emitter.
 */
    Emitter(exports);
 ...
 */
    var attrs = [ "id", "src", "rel", "cols", "rows", "name", "href", "title", "style", "width", "height", "tabindex", "placeholder" ];
    /**
 * Expose `dom()`.
 */
 ...
 */
    ...
    var Confirmation = function(attributes, options) {
        ...
        if (active && active.isShown) active.hide();
        active = this;
    ...
    Confirmation.prototype._render = function() {
        var self = this, el = document.createElement("div"), html = self.template(self.attributes);
 ...
 * @api public
 */
    proto.forEach = proto.each = function(fn) {
 ...
 * @api public
 */
    proto.filter = proto.select = function(fn) {
        fn = toFunction(fn);
        var val;
        var arr = [];
        var vals = this.__iterate__();
 ...
 * Module dependencies.
 */
    var Emitter = require("emitter"), inherit = require("inherit"), each = require("each"), query = require("query"), domify = require("domify"), classes = require("classes"), css = require("css"), events = require("event"), bind = require("bind"), isArray = require("isArray");
    /**
 * Expose `Tip`.
 */
    module.exports = Tip;
...
require.register("bmcmahen-html/index.js", function(exports, require, module) {
    var empty = require("empty"), append = require("append");
    module.exports = function(el, content) {
 ...
 * Module dependencies.
 */
    var Popover = require("popover"), domify = require("domify"), query = require("query"), events = require("event"), bind = require("bind"), html = require("html");
    /**
 ...
 * Module dependencies.
 */
    var Tip = require("tip"), inherit = require("inherit"), domify = require("domify"), classes = require("classes"), query = require("query"), empty = require("empty"), html = require("html");
    /**
 * Expose `Popover`.
 */
 ...
 */
    function Popover(content, title) {
        ...
        if (title) this.title(title); else this.hideTitle();
...
require.register("component-raf/index.js", function(exports, require, module) {
    module.exports = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || fallback;
 ...
 */
    Spinner.prototype.draw = function(ctx) {
        var percent = Math.min(this.percent, 100), ratio = window.devicePixelRatio || 1, size = this.el.width / ratio, half = size / 2, x = half, y = half, rad = half - 1, fontSize = this._fontSize, light = this._light;
        ctx.font = fontSize + "px " + this._font;
        var angle = Math.PI * 2 * (percent / 100);
        ctx.clearRect(0, 0, size, size);
        // outer circle
        var grad = ctx.createLinearGradient(half + Math.sin(Math.PI * 1.5 - angle) * half, half + Math.cos(Math.PI * 1.5 - angle) * half, half + Math.sin(Math.PI * .5 - angle) * half, half + Math.cos(Math.PI * .5 - angle) * half);
        // color
        if (light) {
        ...
        ctx.stroke();
        // text
        var text = this._text || "", w = ctx.measureText(text).width;
...
require.register("eugenicsdatabase/lib/Model.js", function(exports, require, module) {
    var Model = require("model"), request = require("superagent");
    ///////////////////////
    // Eugenics Document //
    ///////////////////////
    var EugenicsDocument = new Model("document").attr("_id").attr("title", {
        required: true,
        type: "string"
        ...
        type: "date"
    }).attr("created_by").attr("modified_by").attr("modified_at", {
        type: "date"
    }).attr("type", {
        type: "string"
    }).attr("prods").attr("resources").attr("image").attr("date").attr("dateOfBirth").attr("dateOfDeath").attr("alternativeName").attr("location").attr("yearOfPublication").attr("monthOfPublication").attr("author").attr("publisher").attr("startDate").attr("endDate").attr("heroQuote").attr("heroQuoteSource").attr("villainQuote").attr("villainQuoteSource").attr("ambiQuote").attr("ambiQuoteSource").attr("latitude").attr("longitude").attr("connections");
    ...
    // Link Model //
    ////////////////
    var LinkModel = new Model("link").attr("_id").attr("from", {
        required: true
    }).attr("to", {
        required: true
    }).attr("strength", {
...
require.register("eugenicsdatabase/lib/Collection.js", function(exports, require, module) {
    var Collection = require("collection"), request = require("superagent"), Emitter = require("emitter"), map = require("map"), type = require("type"), each = require("each");
...
require.register("eugenicsdatabase/lib/BreadCrumbs.js", function(exports, require, module) {
    var dom = require("dom"), each = require("each"), Emitter = require("emitter");
...
require.register("eugenicsdatabase/lib/Controller.js", function(exports, require, module) {
    // MODULES
    var Swipe = require("swipe"), Emitter = require("emitter"), typeOf = require("type"), bind = require("bind"), dom = require("dom"), events = require("event");
    var Collection = require("./Collection"), EugenicsDocument = require("./Model").Document, BreadCrumbs = require("./BreadCrumbs");
    ...
    Controller.prototype.viewDocuments = function(type, name) {
        ...
        if (this.collectionType !== type || this.collectionName !== name) {
            dom("#query").value("");
            this.collection.setUrl(type, name).fetch();
        }
        this.swipe.show(1);
...
require.register("eugenicsdatabase/lib/Document-list.js", function(exports, require, module) {
    // MODULES
    var Cast = require("cast"), Emitter = require("emitter"), bind = require("bind"), type = require("type"), dom = require("dom"), debounce = require("debounce"), each = require("each"), select = require("select"), map = require("map");
    // Imports
    var controller = require("./Controller");
    ...
    SortView.prototype.render = function() {
        ...
        var buildOption = function(val, name) {
            return dom(document.createElement("option")).attr("value", val).text(name);
        };
        ...
        direction.href("#").html('<i class="icon-chevron-down"></i>');
        this.sortWrapper.empty().append(el);
    ...
    collection.on("change", function() {
        ...
        var data = controller.collection.toJSON(), userModel = require("./Model").User, user = userModel && userModel.body;
...
require.register("eugenicsdatabase/lib/Document-summary.js", function(exports, require, module) {
    ...
    var dom = require("dom"), extend = require("extend"), map = require("map"), clone = require("clone");
    // Imports
    var controller = require("./Controller"), template = require("../templates/document-summary");
    ...
    DocumentView.prototype.render = function() {
        ...
        if (json.prods) {
            json.prods = map(json.prods, function(prod, i) {
                if (prodToFullName[prod]) return prodToFullName[prod];
                return prod;
            });
        }
        ...
        if (typeToFullType[json.type]) json.type = typeToFullType[json.type];
...
require.register("eugenicsdatabase/lib/Document-forms.js", function(exports, require, module) {
    // Modules
    var confirmation = require("confirmation"), moment = require("moment"), dom = require("dom"), uniq = require("uniq"), each = require("each"), extend = require("extend"), events = require("events"), contains = require("contains"), type = require("type"), Collection = require("collection"), EmitterManager = require("emitter-manager"), clone = require("clone"), Model = require("model"), enumerable = require("enumerable"), bind = require("bind"), map = require("map"), Confirmation = require("confirmation-popover");
    // Imports
    var controller = require("./Controller"), DocumentModel = require("./Model").Document, Schema = require("./Forms-schema"), Connection = require("./Connections"), Spinner = require("./Spinner");
    ...
    FormGenerator.prototype.generate = function() {
        this.fields = new FieldCollection(this, this.model).on("save", bind(this, this.saveModel)).generateFieldModels();
        var formView = new FormView(this.fields, this, false);
        this.el.empty().append(formView.render().el);
    };
    ...
    FormGenerator.prototype.saveModel = function(json) {
        var previousJSON = this.model.toJSON(), _this = this;
    ...
    // Field Model //
    /////////////////
    var FieldModel = Model("Field").attr("value").attr("label").attr("widget").attr("name").attr("loading").attr("error").validate(function(field) {
    ...
    FieldCollection.prototype.determineFields = function(type, prods) {
        var defaultFields = Schema.required(), toIterate = [];
        toIterate.push(type);
        each(prods, function(prod) {
            toIterate.push(prod);
        });
        each(toIterate, function(required) {
            if (Schema[required]) extend(defaultFields, Schema[required]());
 ...
 */
    FieldCollection.prototype.fieldsToRemove = function(newFields) {
        return this.filter(function(obj, i) {
            if (!newFields[obj.get("name")]) return true;
        }).value();
    };
    /**
 ...
 */
    FieldCollection.prototype.isValid = function() {
        var isValid = true;
        this.each(function(model) {
            if (!model.isValid()) isValid = false;
        });
        return isValid;
    };
    ///////////////
    ...
    FormView.prototype.addChild = function(child) {
        ...
        var saveEl = this.el.find("#save-form").get(), viewEl = view.render().el.get();
        saveEl.parentNode.insertBefore(viewEl, saveEl);
    };
    FormView.prototype.removeChild = function(childModel) {
        var newFieldSet = this.children.reject(function(fieldView) {
 ...
 */
    FormView.prototype.render = function() {
        ...
        var childViews = this.fields.map(function(fieldModel) {
            var view = fieldModel.get("widget") === "formset" ? new FormsetView(fieldModel) : new FieldView(fieldModel);
    ...
    FormView.prototype.buildWidgets = function() {
        if (this.isFormset) {
            this.$remove = dom("<a>[Remove]</a>").href("#").addClass("remove-form");
        } else {
            this.$save = dom("<input>").id("save-form").attr("type", "submit").addClass("btn").addClass("btn-primary").value("Save");
            // If our document isn't new, provide delete button.
            if (!this.context.model.isNew()) {
                this.$delete = dom("<a>Delete</a>").id("delete-form").addClass("delete").addClass("btn").addClass("btn-danger");
 ...
 */
    FormView.prototype.alterType = function(e) {
        var t = e.target, type = dom(t.options[t.selectedIndex]).val();
 ...
 */
    FormView.prototype.alterProds = function(e) {
        var checkbox = dom(e.target).find("input"), isChecked = checkbox.get().checked, name = checkbox.name();
        // for some reason the checked attribute is reversed...
        if (!isChecked) this.fields.addProd(name); else this.fields.removeProd(name);
    };
    /**
 ...
 */
    ...
    var FieldView = function(fieldModel, context) {
        ...
        if (this.attributes.name === "prods") this.el.addClass("prods");
 ...
 */
    FieldView.prototype.render = function() {
        var html = this.widget === "formset" ? "" : this.widgetTemplates[this.widget]({
            object: this.model.toJSON()
        });
        this.el.html(html);
        if (this.widget === "image") {
            var fp = this.el.find("#filepicker").attr("type", "filepicker").get();
 ...
 */
    FieldView.prototype.parse = function() {
        ...
        inputEl = widget === "textarea" ? this.el.find("textarea") : this.el.find("input[name]");
        if (widget === "checkbox") {
            var checkedProds = [];
        ...
        if (widget === "select") {
            ...
            return dom(select.options[select.selectedIndex]).val().toLowerCase();
        }
        return inputEl.val();
    };
    ...
    FieldView.prototype.uploadImage = function(e) {
        var image = e.fpfile, _this = this, currentImage = this.model.value();
        e.preventDefault();
        if (currentImage) {
    ...
    var FormsetView = function(model) {
        this.model = model;
        this.views = [];
        this.el = dom("<div></div>").addClass("formsetView").addClass("well");
        this.buildWidgets();
    ...
    FormsetView.prototype.buildWidgets = function() {
        this.$add = dom("<button></button>").addClass("btn").addClass("btn-small").addClass("add-another").text("Add New Citation");
    };
        ...
        event: function() {
            return {
                date: {
                    ...
                    label: "Date",
                    className: "date",
                    helpText: "Format: YYYY-MM-DD or YYYY",
                    isDate: true
                }
            };
        },
        ...
        place: function() {
            return {
                alternativeNames: {
                    ...
                    helpText: "Enter a comma-separated list of alternative place names."
                },
                location: {
                    widget: "text",
                    label: "Location",
                    helpText: "City, Province"
                }
            };
        },
        ...
        mindmap: function() {
            return {
                connections: {
                    ...
                    helpText: "Click the button to create a connection."
                }
            };
        }
    };
...
require.register("eugenicsdatabase/lib/Connections.js", function(exports, require, module) {
    // Modules
    var Modal = require("modal"), bind = require("bind"), each = require("each"), dom = require("dom"), request = require("superagent"), Collection = require("collection"), events = require("events"), clone = require("clone");
    // Import
    var controller = require("./Controller"), LinkModel = require("./Model").LinkModel;
    // Create our Modal
    ...
    Connection.prototype.fetchAll = function() {
        var _this = this, currentDocument = controller.selectedDocument;
 ...
 */
    Connection.prototype.linkToNode = function(linkView) {
        ...
        var attr;
        if (model.from()) {
            attr = current._id() === model.from()._id ? model.to() : model.from();
        } else {
            attr = model.to();
        }
 ...
 */
    Connection.prototype.nodeToLink = function(nodeView) {
        var attr = nodeView.attr, current = controller.selectedDocument;
        // remove our element
        nodeView.close();
        ...
        if (current && current._id()) model.from({
            title: current.title(),
            _id: current._id()
        });
        model.strength(1);
 ...
 */
    var LinkNode = function(model, context) {
        ...
        var current = controller.selectedDocument;
        if (model.from()) {
            this.attr = current._id() === model.from()._id ? clone(model.to()) : clone(model.from());
        } else {
    ...
    LinkNode.prototype.parse = function() {
        var select = this.$el.find("select").get(), val = dom(select.options[select.selectedIndex]).val();
...
require.register("eugenicsdatabase/lib/Spinner.js", function(exports, require, module) {
    ...
    module.exports = {
        show: function() {
            if (appended) return;
            this.spinner = new Spinner().size(30).light();
        ...
        with (locals || {}) {
            if (locals.image) {
                buf.push("<img" + jade.attrs({
                    width: "40px",
                    height: "40px",
                    src: image.url + "/convert?w=40&h=40&fit=crop&align=faces"
                }, {
                    width: true,
                    height: true,
                    src: true
...
require.register("eugenicsdatabase/templates/document-summary.js", function(exports, require, module) {
    module.exports = function anonymous(locals) {
        ...
        with (locals || {}) {
            ...
            if (locals.image) {
                buf.push("<dt>Image</dt><dd><img" + jade.attrs({
                    src: image.url + "/convert?w=300&h=200&fit=max",
                    "class": "document-summary-image" + " " + "img-polaroid"
                }, {
                    src: true
                }) + "/></dd>");
            }
...
require.register("eugenicsdatabase/templates/button.js", function(exports, require, module) {
    module.exports = function anonymous(locals) {
        ...
        with (locals || {}) {
            ...
            buf.push('<span class="target-link"></span><div><button' + jade.attrs({
                name: object.name,
                "class": "btn" + " " + "btn-success"
            }, {
                name: true
            }) + ">Edit Connections</button></div></label>");
            if (object.error) {
                    ...
                    for (var $index = 0, $$l = $$obj.length; $index < $$l; $index++) {
                        ...
                        (function() {
                            ...
                            if ("number" == typeof $$obj.length) {
                                for (var $index = 0, $$l = $$obj.length; $index < $$l; $index++) {
                                    var f = $$obj[$index];
                                    buf.push('<label class="prod"><input' + jade.attrs({
                                        type: "checkbox",
                                        name: f.name,
                                        checked: f.value
                                    }, {
                                        ...
                                        name: true,
                                        checked: true
                                    }) + "/>" + jade.escape((jade.interp = f.label) == null ? "" : jade.interp) + "</label>");
                                }
                            } else {
                                var $$l = 0;
                                for (var $index in $$obj) {
...
require.register("eugenicsdatabase/templates/image.js", function(exports, require, module) {
    module.exports = function anonymous(locals) {
        ...
        with (locals || {}) {
            ...
            if (object.value) {
                buf.push("<img" + jade.attrs({
                    src: object.value.url + "/convert?w=250&h=250",
    ...
    module.exports = function anonymous(locals) {
        ...
        with (locals || {}) {
            buf.push("<label" + jade.attrs({
                ...
                "class": true
            }) + ">" + jade.escape(null == (jade.interp = object.label) ? "" : jade.interp) + "<select>");
            (function() {
                ...
                if ("number" == typeof $$obj.length) {
                    for (var $index = 0, $$l = $$obj.length; $index < $$l; $index++) {
                        ...
                        buf.push("<option" + jade.attrs({
                            value: option.name,
                            selected: option.name == object.value
                        }, {
                            value: true,
                            selected: true
                        }) + ">" + jade.escape(null == (jade.interp = option.label) ? "" : jade.interp) + "</option>");
                    }
                } else {
                    var $$l = 0;
                    for (var $index in $$obj) {
    ...
    module.exports = function anonymous(locals) {
        ...
        with (locals || {}) {
            ...
            buf.push("<textarea" + jade.attrs({
                ...
                name: true,
                "class": true
            }) + ">" + jade.escape(null == (jade.interp = object.value) ? "" : jade.interp) + "</textarea></label>");
            if (object.error) {
                buf.push('<p class="error-text">' + jade.escape(null == (jade.interp = object.error) ? "" : jade.interp) + "</p>");
            }
            if (object.helpText) {
...
require.register("eugenicsdatabase/templates/connection-select.js", function(exports, require, module) {
    module.exports = function anonymous(locals) {
        var buf = [];
        with (locals || {}) {
            buf.push('<span class="document-name">' + jade.escape(null == (jade.interp = title) ? "" : jade.interp) + '</span><button class="btn btn-small btn-warning remove">Remove</button><select class="strength"><option' + jade.attrs({
                value: "1",
                selected: strength === 1
            }, {
                value: true,
                selected: true
            }) + ">1</option><option" + jade.attrs({
                value: "2",
                selected: strength === 2
            }, {
                value: true,
                selected: true
            }) + ">2</option><option" + jade.attrs({
                value: "3",
                selected: strength === 3
            }, {
                value: true,
                selected: true
            }) + ">3</option><option" + jade.attrs({
                value: "4",
                selected: strength === 4
            }, {
                value: true,
                selected: true
            }) + ">4</option><option" + jade.attrs({
                value: "5",
                selected: strength === 5
            }, {
                value: true,
                selected: true
            }) + ">5</option><option" + jade.attrs({
                value: "6",
                selected: strength === 6
            }, {
                value: true,
                selected: true
            }) + ">6</option><option" + jade.attrs({
                value: "7",
                selected: strength === 7
            }, {
                value: true,
                selected: true
            }) + ">7</option><option" + jade.attrs({
                value: "8",
                selected: strength === 8
            }, {
                value: true,
                selected: true
            }) + ">8</option><option" + jade.attrs({
                value: "9",
                selected: strength === 9
            }, {
                value: true,
                selected: true
            }) + ">9</option><option" + jade.attrs({
                value: "10",
