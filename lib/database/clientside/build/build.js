...
var EugenicsDocument = new Model('document')
  ...
  .attr('modified', { type: 'date' });

EugenicsDocument.base = 'api/document';
...
DocumentCollection.prototype.fetch = function(url){
  var _this = this;
  url = url || this.url;
...
BreadCrumbItem.prototype.isLast = function(){
 return this.context.models.length === (this.i + 1) ? true : false;
};
...
Controller.prototype.editDocument = function(id){
  var selectedDocument = this.findDocumentById(id);
  if (selectedDocument){
    this.selectedDocument = selectedDocument;
    this.emit('editDocument', selectedDocument);
    this.swipe.show(3);
    this.breadCrumbs.add('Edit', 3);
...
Controller.prototype.url = function(url){
  console.log('hi router, go here', url);
};

module.exports = new Controller();

var DocumentList = require('./Document-list');
var DocumentSummary = require('./Document-summary');
...
CastListing.prototype.onViewRendered = function(view){
  dom(view.el).find('a.edit-btn').on('click', function(e){
    e.preventDefault();
    controller.editDocument(view.model.toJSON());
  });
...
collection.on('reset', function(){
  ...
  //   attr.currentUser = require('./current-user');
  //   return attr;
  // });
...
// XXX todo - listen for model changes!
...
// Imports
var controller = require('./Controller')
	, template = require('../templates/document-summary');
...
DocumentView.prototype.bind = function(){
	...
	dom('#edit-document').on('click', function(e){
		e.preventDefault();
		controller.editDocument(_this.model._id());
	});
};
...
// Imports
var controller = require('./Controller')
  , DocumentModel = require('./Model').Document
...
FormGenerator.prototype.deleteDocument = function(){
  ...
  // Use the destory method, but also handle our views.
};
...
FieldCollection.prototype.generateFieldModels = function(){
  ...
  var fields = this.determineFields(attr.type, attr.prods);
...
FormView.prototype.bind = function(){
  this.fields
    .on('add', bind(this, this.addChild))
    .on('remove', bind(this, this.removeChild));

  this.el
    .on('submit', bind(this, this.saveForm));
};
...
FormView.prototype.removeFormElement = function(e){
  e.preventDefault();
  //console.log('remove', this.context);
...
FormView.prototype.bindWidgets = function(){
  this.el
    .find('select')
    .on('change', bind(this, this.alterType));
  this.el
    .find('.prod')
    .on('click', bind(this, this.alterProds));
};
...
FormView.prototype.buildWidgets = function(){
  if (this.isFormset){
    ...
    this.$remove = dom('<a>[Remove]</a>')
      ...
      .on('click', bind(this, this.removeFormElement));
    ...
    if (! this.context.model.isNew()) {
      this.$delete = dom('<a>Delete</a>')
        ...
        .addClass('btn')
        .addClass('btn-danger')
        .on('click', bind(this, this.deleteDocument));
    }
  }
};

FormView.prototype.events = function(){
  this.el.on('submit', bind(this, this.saveForm));

  // These should probably be on field level...
  this.el
    .find('select')
    .on('change', bind(this, this.alterType));
...
FormView.prototype.parse = function(){
  this.children.each(function(fieldView){
    var widget = fieldView.model.get('widget')
      , value = fieldView.parse();

    if (widget !== 'image') {
      fieldView.model.value(value);
    }
  });
...
FormView.prototype.alterProds = function(e){
  var checkbox = dom(e.currentTarget).find('input')
...
FormView.prototype.deleteDocument = function(e){
  ...
  require('eugenicsarchivesca-popover').effect = 'fade';
  confirm.focus('ok');
  confirm.ok('Delete Document');
  confirm.show(e.currentTarget, function(ok){
    _this.context.deleteDocument();
  });
};
...
var FieldView = function(fieldModel){
...
FieldView.prototype.listen = function(){
  this.model.on('change error', bind(this, this.render));
...
FieldView.prototype.render = function(){
  ...
  if (this.widget === 'image'){
    var fp = this.el
      ...
      .on('change', bind(this, this.uploadImage))
      .get();
    filepicker.constructWidget(fp);
    this.el
      .find('#remove-image')
      .on('click', bind(this, this.removeImage));
  }
...
var FormsetView = function(model){
  ...
  if (model.get('value')){
    ...
    this.formCollection.add(new FormModel(fields));
  }
...
FormsetView.prototype.close = function(){
  console.log('close formset!?');
};
...
module.exports = {
  ...
  person : function() {
    return {
      dateOfBirth: { widget: 'text', label: 'Date of Birth', className: 'dateOfBirth', helpText: 'Format: YYYY-MM-DD or YYYY', isDate: true},
      dateOfDeath: { widget: 'text', label: 'Date of Death', className: 'dateOfDeath', helpText: 'Format: YYYY-MM-DD or YYYY', isDate: true}
    };
  },
...
require.alias("esundahl-spin.js/index.js", "eugenicsdatabase/deps/spin.js/index.js");
require.alias("esundahl-spin.js/spin.js", "eugenicsdatabase/deps/spin.js/spin.js");
require.alias("esundahl-spin.js/index.js", "spin.js/index.js");
...
require.alias("MatthewMueller-hogan/hogan.js", "eugenicsdatabase/deps/hogan/hogan.js");
require.alias("MatthewMueller-hogan/index.js", "eugenicsdatabase/deps/hogan/index.js");
require.alias("MatthewMueller-hogan/index.js", "hogan/index.js");
